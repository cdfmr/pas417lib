...
{$ENDIF}
  ...
  TPDF417BarcodeVCL = class({$IFDEF ACTIVEX}TCustomControl{$ELSE}TGraphicControl{$ENDIF})
  private
{$IFDEF ACTIVEX}
    FLines: string;
{$ELSE}
...
{$ELSE}
procedure TPDF417BarcodeVCL.SetLines(Value: TStrings);
begin
  FLines.Assign(Value);
end;
{$ENDIF}
...
{$IFNDEF ACTIVEX}
procedure TPDF417BarcodeVCL.DummyBitmap(Value: TBitmap);
begin
{Dummy Procedure}
end;

procedure TPDF417BarcodeVCL.TextChanged(Sender: TObject);
begin
  UpdateBarcode;
end;
{$ENDIF}
...
function TPDF417BarcodeVCL.DrawTo(DC: HDC; X, Y: Integer; Zoom: Integer; AutoZoom: Boolean): Boolean;
var
  ZoomX, ZoomY: Integer;
begin
  Result := False;
...
function TPDF417BarcodeVCL.DrawToRect(DC: HDC; R: TRect; Zoom: Integer; AutoZoom: Boolean): Boolean;
var
  ZoomX, ZoomY: Integer;
  X, Y: Integer;
begin
