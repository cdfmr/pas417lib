...
procedure listAdd(list: pArrayList; ctype: Char; cstart, cend: Integer);
var
  temp: pListElement;
begin
...
function checkElementType(p: pListElement; ctype: char): Integer;
begin
...
begin
  ...
  p.param.outBits[bytePtr] := Chr(Ord(p.param.outBits[bytePtr]) or (codeword shr (9 + bit)));
  Inc(bytePtr);
  p.param.outBits[bytePtr] := Chr(Ord(p.param.outBits[bytePtr]) or (codeword shr (1 + bit)));
  Inc(bytePtr);
...
begin
  bytePtr := p.bitPtr div 8;
  bit := p.bitPtr - bytePtr * 8;
  p.param.outBits[bytePtr] := Chr(Ord(p.param.outBits[bytePtr]) or (codeword shr (10 + bit)));
  Inc(bytePtr);
  p.param.outBits[bytePtr] := Chr(Ord(p.param.outBits[bytePtr]) or (codeword shr (2 + bit)));
  Inc(bytePtr);
  codeword := codeword shl 8;
  p.param.outBits[bytePtr] := Chr(Ord(p.param.outBits[bytePtr]) or (codeword shr (2 + bit)));
  if bit = 7 then
  begin                                                          
    Inc(bytePtr);
    p.param.outBits[bytePtr] := Chr(Ord(p.param.outBits[bytePtr]) or $80);
  end;
  Inc(p.bitPtr, 18);
end;
...
begin
  ...
  p.param.outBits := PChar(AllocMem(p.param.lenBits));
  ...
  if p.param.options and PDF417_INVERT_BITMAP <> 0 then
    for i := 0 to p.param.lenBits - 1 do
      p.param.outBits[i] := Chr(Ord(p.param.outBits[i]) xor $ff);
end;
...
function getTextTypeAndValue(text: PChar; size: Integer; idx: Integer): Integer;
var
  c: Char;
  ms, ps: Integer;
begin
